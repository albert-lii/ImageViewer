...
package com.liyi.viewer.progressbar;
...
//        </attr>
        ...
        // 进度条的圆形背景（如果控件设置了background，则此属性无效）
        mBackGroundColor = a.getColor(
                R.styleable.CircleProgressBar_mlpb_background_color, DEFAULT_CIRCLE_BG_LIGHT);
        // 进度条的颜色
        mProgressColor = a.getColor(
                R.styleable.CircleProgressBar_mlpb_progress_color, DEFAULT_CIRCLE_LIGHT);
        // 圆形进度条的半径（不能大于控件直径的1/2-mProgressStokeWidth，否则不显示）
        ...
        mCircleBackgroundEnabled = a.getBoolean(
                R.styleable.CircleProgressBar_mlpb_enable_circle_background, true);
        // 进度文字（百分比）
        mProgress = a.getInt(
                R.styleable.CircleProgressBar_mlpb_progress, 0);
        // 进度的最大值
        mMax = a.getInt(
                R.styleable.CircleProgressBar_mlpb_max, 100);
        // 是否显示文字
